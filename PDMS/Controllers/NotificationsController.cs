...
namespace PDMS.Controllers
{
    ...
    public class NotificationsController : Controller
    {
        ...
        [HttpGet("list")]
        [Authorize(Roles = $"{RolesConstants.DIRECTOR},{RolesConstants.SUPERVISOR},{RolesConstants.ACCOUNTANT},{RolesConstants.SALEMAN}")]
        public async Task<ActionResult<List<NotificationDto>>> GetNotifications()
        {
            List<Notification> notifications = await _context.Notifications
                .Include(x => x.CustomerCreate)
        ...
        [Authorize(Roles = $"{RolesConstants.DIRECTOR},{RolesConstants.SUPERVISOR},{RolesConstants.ACCOUNTANT},{RolesConstants.SALEMAN}")]
        public async Task<ActionResult<List<NotificationDto>>> DeleteAllNotification()
        {
            var notifications = await _context.Notifications.Where(x => x.Status).ToListAsync();
            if (notifications == null)
            {
                return ValidationError.BadRequest400($"Notification is not exist");
            }
            notifications.ForEach(x => x.Status = false);
            var result = await _context.SaveChangesAsync();
            if (result == 0)
            {
                return ValidationError.InternalServerError500("Error occur while delete notification");

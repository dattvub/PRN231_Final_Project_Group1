const options = {
    valueNames: [
        'customerCode',
        'customerName',
        {
            data: ['customerId'],
        }
    ],
    page: 10
}
const initValues = [
    {
        customerId: 1,
        customerCode: 'ABC',
        customerName: 'Pfizer',
        status: true,
    },
    {
        customerId: 2,
        customerCode: 'ABC',
        customerName: 'AbbVie',
        status: true,
    },
    {
        customerId: 3,
        customerCode: 'ABC',
        customerName: 'Johnson & Johnson',
        status: true,
    },
    {
        customerId: 4,
        customerCode: 'ABC',
        customerName: 'Merck & Co',
        status: true,
    },
    {
        customerId: 5,
        customerCode: 'ABC',
        customerName: 'Novartis',
        status: true,
    },
    {
        customerId: 6,
        customerCode: 'ABC',
        customerName: 'Roche',
        status: true,
    },
    {
        customerId: 7,
        customerCode: 'ABC',
        customerName: 'Bristol-Myers Squibb',
        status: true,
    },
    {
        customerId: 8,
        customerCode: 'ABC',
        customerName: 'Sanofi',
        status: true,
    },
    {
        customerId: 9,
        customerCode: 'ABC',
        customerName: 'AstraZeneca',
        status: true,
    },
    {
        customerId: 10,
        customerCode: 'ABC',
        customerName: 'GSK',
        status: true,
    },
]
const formTitle = $('#formTitle')
const defaultFormTitle = formTitle.text()
const customerList = new List('customerList', options)
const mainCard = $('#mainCard')
const newBtnGroup = $('#newBtnGroup')
const customerForm = $('#customerForm')
const customerIdInput = $('#customerId')
const customerCodeInput = $('#customerCode')
const customerNameInput = $('#customerName')
/** @type {[{id: number, type: string}]} */
const highlightList = []
let newOpened = mainCard.hasClass('newOpened');
let currentPage = 1
let currentTotalPage = 1
let searchKeyword = ''
let searchTimer
let handleByPaste = false

async function loadcustomers(page, search) {
    page = page || 1
    const params = new URLSearchParams()
    params.append('page', page)
    params.append('quantity', '10')
    if (search && search.trim().length > 0) {
        params.append('query', search)
        params.append('queryByName', $('input[name=searchBy]:checked').val())
    }
    $.get(`http://localhost:5000/customer/list?${params}`)
        .done(result => {
            currentPage = page
            $('#mainGrid tbody .delete-btn').each((i, elm) => {
                bootstrap.Popover.getInstance(elm).dispose()
            })
            customerList.clear()
            customerList.add(result.items, onLoadcustomerList)
            const totalPage = Math.max(Math.ceil(result.total / result.itemsPerPage), 1)
            currentTotalPage = totalPage
            if (page > totalPage) {
                page = totalPage
                loadcustomers(page, search)
                return
            }
            renderPaginationNumber(currentPage, totalPage)
        })
        .fail(err => {
            console.log(err)
        })
}

function renderPaginationNumber(active, totalPage) {
    if (active < 1 || active > totalPage) {
        return
    }

    let limit = 6, lower = active, upper = active
    while (limit > 1 && (lower > 1 || upper < totalPage)) {
        if (lower > 1) {
            lower--;
            limit--;
        }
        if (limit <= 1) {
            break;
        }
        if (upper < totalPage) {
            upper++;
            limit--;
        }
    }

    const paginationGroup = $('#paginationBar > .pagination-number-group')
    paginationGroup.html('')

    for (let i = lower; i <= upper; i++) {
        const page = i
        const paginationItem = $('<li></li>').attr({'data-page': page})
        if (currentPage === i) {
            paginationItem
                .addClass('active fw-medium text-primary mx-1 px-2 d-flex align-items-center')
                .text(page)
        } else {
            paginationItem.append(
                $('<button></button>').attr({
                    class: 'page',
                    type: 'button',
                }).text(page).on('click', () => {
                    loadcustomers(page, searchKeyword)
                })
            )
        }
        paginationGroup.append(paginationItem)
    }

    paginationGroup.find(`[data-page=${active}]`).addClass('active')
    $('#paginationBar > .pagination-prev').toggleClass('disabled', active === 1)
    $('#paginationBar > .pagination-next').toggleClass('disabled', active === totalPage)
}

function formClear() {
    customerForm.removeClass('was-validated')
    customerIdInput.val('')
    customerForm[0].reset()
}

function toggleNewForm() {
    newOpened = !newOpened
    if (!newOpened) {
        formClear()
        mainCard.removeClass('updating')
        formTitle.text(defaultFormTitle)
        $('#customerList').find('.actions > button').removeClass('disabled')
    }
    mainCard.toggleClass('newOpened', newOpened)
}

function loadcustomerIntoForm(customer) {
    customerIdInput.val(customer.customerId)
    customerCodeInput.val(customer.customerCode)
    customerNameInput.val(customer.customerName)
}

function trimInputElement(e) {
    e.stopPropagation();
    const inputElement = e.delegateTarget
    const cursorPos = inputElement.selectionStart
    const beforeTrim = inputElement.value
    inputElement.value = beforeTrim.trim()
    if (beforeTrim.length > inputElement.value.length) {
        inputElement.selectionStart = cursorPos - 1
        inputElement.selectionEnd = cursorPos - 1
    }
}

function onLoadcustomerList(items) {
    items.forEach(item => {
        const itemId = item.values().customerId
        const rowElm = $(item.elm)

        rowElm.find('.edit-btn').on('click', () => {
            if (newOpened && !customerIdInput.val()) {
                return
            }

            $('#formTitle').text(`Cập nhật: ${item.values().customerName}`)
            loadcustomerIntoForm(item.values())
            $('#customerList').find('.delete-btn').addClass('disabled')
            if (!newOpened) {
                mainCard.addClass('updating')
                toggleNewForm()
            }
        })

        function deletecustomer() {
            if (!popOver) {
                return
            }
            $.ajax({
                type: 'DELETE',
                url: `http://localhost:5000/customer/${itemId}`,
                dataType: 'json',
                contentType: 'application/json',
            })
                .done(result => {
                    console.log(result)
                    loadcustomers(currentPage, searchKeyword)
                })
                .fail(err => {
                    console.log(err)
                })
        }

        const popOverElement = $('<div></div>').append([
            $('<p class="mb-2"></p>')
                .append($('<span></span>').text('Xác nhận xoá customer '))
                .append($('<strong></strong>').text(item.values().customerCode)),
            $('<button class="btn btn-sm btn-danger w-50 mx-auto d-block">Xoá</button>').on('click', deletecustomer),
        ])
        const popOverOptions = {
            html: true,
            trigger: 'click',
            placement: 'left',
            content: popOverElement
        }
        const popOver = new bootstrap.Popover(rowElm.find('.delete-btn'), popOverOptions)

        const index = highlightList.findIndex(x => x.id === itemId)
        if (index >= 0) {
            const className = `row-${highlightList[index].type}`
            highlightList.splice(index, 1)
            rowElm.addClass(className)
            setTimeout(() => {
                rowElm.removeClass(className)
            }, 300)
        }
    })
}

customerCodeInput.on({
    keypress: e => {
        const key = String.fromCharCode(e.charCode || e.which)
        if (!/^[a-zA-Z0-9]+$/.test(key)) {
            e.preventDefault()
            return false
        }
    },
    change: e => {
        customerCodeInput.val(customerCodeInput.val().replace(/[^a-zA-Z0-9]+/g, ''))
    }
})

customerNameInput.on('input', trimInputElement)

customerList.clear()
loadcustomers()

newBtnGroup.on('click', () => {
    $('#customerList').find('.actions > button').addClass('disabled')
    toggleNewForm()
})

$('#paginationBar .pagination-prev').on('click', () => {
    if (currentPage <= 1) {
        return
    }

    loadcustomers(--currentPage, searchKeyword)
})

$('#paginationBar .pagination-next').on('click', () => {
    if (currentPage >= currentTotalPage) {
        return
    }

    loadcustomers(++currentPage, searchKeyword)
})

$('#searchInput').on({
    input: e => {
        if (handleByPaste) {
            handleByPaste = false
            return
        }

        const keyword = e.delegateTarget.value

        if (searchTimer) {
            clearTimeout(searchTimer)
        }

        searchTimer = setTimeout(() => {
            if (keyword === searchKeyword) {
                return
            }
            searchKeyword = keyword
            loadcustomers(1, searchKeyword)
        }, keyword ? 400 : 0)
    },
    paste: e => {
        handleByPaste = true

        if (searchTimer) {
            clearTimeout(searchTimer)
        }

        if (!e.originalEvent.clipboardData.types.includes('text/plain')) {
            return
        }

        searchKeyword = e.originalEvent.clipboardData.getData('Text')
        loadcustomers(1, searchKeyword)
    }
})

$('#clearSearchBtn').on('click', () => {
    $('#searchInput').val('').trigger('input')
})

$(':is(#byName, #byCode)').on('change', () => {
    if (!searchKeyword) {
        return
    }
    loadcustomers(1, searchKeyword)
})

customerForm.submit((e) => {
    e.preventDefault()
    e.stopPropagation()

    if (customerForm.find(':invalid').length > 0) {
        customerForm.addClass('was-validated')
        return
    }

    const data = {
        customerCode: customerCodeInput.val().trim().toUpperCase(),
        customerName: customerNameInput.val().trim()
    }
    const customerId = parseInt($('#customerId').val())
    const url = customerId ? `http://localhost:5000/customer/${customerId}` : 'http://localhost:5000/customer/create'
    const method = customerId ? 'PUT' : 'POST'

    if (data.customerCode.length === 0 || data.customerName.length === 0) {
        return
    }

    $('#customerForm > fieldset').prop('disabled', true)
    $.ajax({
        type: method,
        url,
        dataType: 'json',
        contentType: 'application/json',
        data: JSON.stringify(data),
    })
        .done(result => {
            console.log(result)
            if (newOpened) {
                toggleNewForm()
                formClear()
            }
            highlightList.push({
                id: result.customerId,
                type: customerId ? 'info' : 'success'
            })
            loadcustomers(currentPage, searchKeyword)
        })
        .fail(err => {
            console.log(err)
        })
        .always(() => {
            $('#customerForm > fieldset').prop('disabled', false)
        })
})